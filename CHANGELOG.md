...
- Add `"helm.sh/resource-policy": keep` annotation to the provider secret. This is to ensure that it isn't removed by Helm, thus leading to a race condition when deleting the cluster as the vsphere cleaner job needs it to clean up resources in vSphere.
